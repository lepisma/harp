<script lang="ts">
  ...
  import type { Asset, Source, Document } from '$lib/types';
  ...
  let { onSave, onClose, title = 'New Document', onAssetUpload, doc = null } = $props();
  ...
  if (doc !== null) {
    name = doc.name;
    datetime = formatDateForInput(doc.datetime);
    uuid = doc.uuid;
    tags = doc.tags;
    source = doc.source;
    assets = doc.assets;
